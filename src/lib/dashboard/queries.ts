...
function toSeries(
  rows: SnapshotRow[],
  dateKeys: string[],
  extract: (metrics: unknown) => number | null,
): DailyPoint[] {
...
function safeExtract<TData>(
  schema: { safeParse: (v: unknown) => { success: boolean; data?: TData } },
  metrics: unknown,
...
export async function getDashboardData(
  ...
  // window edges. Historical responses are reduced to the latest attempt
  // per client/platform in SQL instead of transferring their full history.
  const fetchSince = subDays(now, SPARKLINE_DAYS + 3);
  const fetchSinceKey = format(fetchSince, "yyyy-MM-dd");

  const [snapshotRows, rawRows, mappingRows, successfulSyncRows] =
    clientIds.length
      ? await Promise.all([
          db
            .select()
            ...
            .where(
              and(
                ...
                gte(metricSnapshots.date, fetchSinceKey),
              ),
            ),
          db
            .selectDistinctOn([rawResponses.clientId, rawResponses.platform])
            .from(rawResponses)
            .where(inArray(rawResponses.clientId, clientIds))
            .orderBy(
              rawResponses.clientId,
              rawResponses.platform,
              desc(rawResponses.fetchedAt),
            ),
          db
            .select()
            .from(clientPlatformAccounts)
            .where(inArray(clientPlatformAccounts.clientId, clientIds)),
          db
            .select({
              clientId: metricSnapshots.clientId,
              at: sql<string | null>`max(${metricSnapshots.createdAt})`,
            })
            .from(metricSnapshots)
            .where(inArray(metricSnapshots.clientId, clientIds))
  ...
  const successfulSyncByClient = new Map(
    successfulSyncRows.map((row) => [
      row.clientId,
      row.at ? new Date(row.at) : null,
    ]),
  );
  const attemptByClient = new Map<string, Date>();
  for (const row of rawRows) {
    if (
      !attemptByClient.has(row.clientId) ||
      row.fetchedAt > attemptByClient.get(row.clientId)!
    )
      attemptByClient.set(row.clientId, row.fetchedAt);
  }
  ...
  for (const client of activeClients) {
    ...
    const leadsRows = rowsFor(
      snapshotsByClient,
      client.id,
      "lead_dashboard",
      current7,
    );
    const leadsPrevRows = rowsFor(
      snapshotsByClient,
      client.id,
      "lead_dashboard",
      prev7,
    );
    const leadsErr = attemptErrorFor(
      latestAttempt,
      client.id,
      "lead_dashboard",
    );
    ...
    const calls = downgradeIfUnverifiedMapping(
      buildCallsCell(callsRows, telephonyDataSchema, callsErr),
      callsVerified,
    );
    ...
    const googleAdsRows = rowsFor(
      snapshotsByClient,
      client.id,
      "google_ads",
      current7,
    );
    const googleAdsErr = attemptErrorFor(
      latestAttempt,
      client.id,
      "google_ads",
    );
    ...
    const conversions = downgradeIfUnverifiedMapping(
      buildNumericCell(
        ga4Rows,
        ga4DataSchema,
        (d) => d.totalConversions,
        ga4Err,
      ),
      ga4Verified,
    );

    const searchConsoleRows = rowsFor(
      snapshotsByClient,
      client.id,
      "search_console",
      current7,
    );
    const searchConsoleErr = attemptErrorFor(
      latestAttempt,
      client.id,
      "search_console",
    );
    ...
    const lastSyncedAt = successfulSyncByClient.get(client.id) ?? null;
    const staleHours = lastSyncedAt
      ? differenceInHours(now, lastSyncedAt)
      : null;
    ...
    rows.push({
      ...
      avgPosition,
      lastSyncedAt,
      lastAttemptAt: attemptByClient.get(client.id) ?? null,
      staleHours,
    });
    ...
    const sparklines: SparklineMetric[] = [
      {
        ...
        label: "Leads",
        points: toSeries(
          rowsFor(snapshotsByClient, client.id, "lead_dashboard", sparkline30),
          sparkline30,
          (m) => safeExtract(leadDashboardDataSchema, m, (d) => d.totalLeads),
        ),
      },
      {
        key: "callsTotal",
        label: "Calls",
        points: toSeries(
          rowsFor(snapshotsByClient, client.id, "openphone", sparkline30),
          sparkline30,
          (m) => safeExtract(telephonyDataSchema, m, (d) => d.totalCalls),
        ),
      },
      {
        key: "callsMissed",
        ...
        points: toSeries(
          ...
          (m) =>
            safeExtract(
              telephonyDataSchema,
              m,
              (d) => d.missedCalls - d.missedAndForwardedCalls,
            ),
        ),
      },
      {
        ...
        points: sparkline30.map((date) => {
          const gRows = rowsFor(snapshotsByClient, client.id, "google_ads", [
            date,
          ]);
          const mRows = rowsFor(snapshotsByClient, client.id, "meta", [date]);
          if (gRows.length === 0 && mRows.length === 0)
            return { date, value: null };
          const g = gRows.length
            ? safeExtract(googleAdsDataSchema, gRows[0].metrics, (d) => d.cost)
            : null;
          const m = mRows.length
            ? safeExtract(metaDataSchema, mRows[0].metrics, (d) => d.spend)
            : null;
          return {
            date,
            value: g === null && m === null ? null : (g ?? 0) + (m ?? 0),
          };
        }),
      },
      {
        ...
        label: "Sessions",
        points: toSeries(
          rowsFor(snapshotsByClient, client.id, "ga4", sparkline30),
          sparkline30,
          (m) => safeExtract(ga4DataSchema, m, (d) => d.totalSessions),
        ),
      },
      {
        key: "conversions",
        ...
        points: toSeries(
          rowsFor(snapshotsByClient, client.id, "ga4", sparkline30),
          sparkline30,
          (m) => safeExtract(ga4DataSchema, m, (d) => d.totalConversions),
        ),
      },
      {
        key: "avgPosition",
        label: "Avg. position",
        points: toSeries(
          rowsFor(snapshotsByClient, client.id, "search_console", sparkline30),
          sparkline30,
          (m) =>
...
export async function getSyncStatus(
  ...
  const lastSyncRows = await db
    .select({
      ...
      lastSync: sql<string | null>`max(${metricSnapshots.createdAt})`,
    })
    .from(metricSnapshots)
  ...
  const errorsInLastRun = latestRun
    ? await db
        .select()
        .from(rawResponses)
        .where(eq(rawResponses.syncRunId, latestRun.id))
    : [];

  const lastSyncByPlatform = new Map<Platform, Date | null>();
  for (const row of lastSyncRows) {
    lastSyncByPlatform.set(
      row.platform,
      row.lastSync ? new Date(row.lastSync) : null,
    );
  }
  ...
  for (const row of verifiedRows) {
    const entry = verifiedByPlatform.get(row.platform) ?? {
      verified: 0,
      unverified: 0,
    };
  ...
  for (const row of errorsInLastRun) {
    if (isErrorPayload(row.payload)) {
      errorCountByPlatform.set(
        row.platform,
        (errorCountByPlatform.get(row.platform) ?? 0) + 1,
      );
    }
  }
