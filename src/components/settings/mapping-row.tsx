...
function StatusBadge({ mapping }: { mapping: MappingRowData | null }) {
  ...
  if (mapping.verifiedStatus === "ok") {
    return (
      <Badge
        variant="outline"
        className="gap-1 border-emerald-600/30 text-emerald-700 dark:text-emerald-500"
      >
  ...
  if (mapping.verifiedStatus === "no_data") {
    return (
      <Badge
        variant="outline"
        className="gap-1 border-amber-600/30 text-amber-700 dark:text-amber-500"
      >
  ...
  if (result.status === "no_data") {
    return (
      <p className="text-xs text-amber-700 dark:text-amber-500">
        Connected, but returned nothing for this period.
      </p>
    );
  }
  ...
  if (entries.length === 0) {
    return (
      <p className="text-xs text-emerald-700 dark:text-emerald-500">
        Connected — no scalar figures to show.
      </p>
    );
  }
  return (
...
export function MappingRow({
  ...
  const liveCheck =
    externalId.trim().length > 0
      ? externalIdSchemas[platform].safeParse(externalId)
      : null;
  const liveError =
    liveCheck && !liveCheck.success ? liveCheck.error.issues[0]?.message : null;
  ...
  return (
    <div className="flex flex-col gap-2.5 border-b border-border px-4 py-3.5 last:border-b-0">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {mapping?.verifiedAt && (
          <span className="text-xs text-muted-foreground">
            Last checked{" "}
            {mapping.verifiedAt.toLocaleString("en-US", { timeZone: "UTC" })}{" "}
            UTC
          </span>
        )}
      </div>
      ...
      >
        <div className="flex min-w-0 basis-full flex-col gap-1 sm:min-w-64 sm:basis-0 sm:flex-1">
          <AccountCombobox
            platform={platform}
            name="externalId"
            value={externalId}
        ...
        <div className="flex items-center gap-1.5 pt-1.5">
          <Switch
            aria-label={`Include ${label} in daily syncs`}
            name="active"
            value="true"
            checked={active}
        ...
        <div className="flex items-center gap-2 pt-0.5">
          <Button
            ...
            size="sm"
            variant="secondary"
            disabled={savePending || externalId.trim().length === 0}
          >
